...
async def call_openai_api(client_config: Dict, prompt: str) -> Dict:
    try:
        ...
        # 显式检查模型可用性
        models = [m.id for m in await client.models.list()]
        if client_config["model"] not in models:
            return {"model": client_config["name"], "content": f"模型不存在，可用模型：{models}", "success": False}
...
async def call_ollama_api(model_config: Dict, prompt: str) -> Dict:
    try:
        ...
        # Ollama特殊处理：强制移除API_KEY检查
        client._api_key = None  # 关键修正：禁用OpenAI SDK的密钥验证
